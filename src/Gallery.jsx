...
export default function Gallery() {
  ...
  const t = {
    ...
    title: 'DENIA GALLERY',
    disclaimer: lang === 'zh' ? '本网站为非盈利性质的个人爱好项目。网站将保持更新。' : 'This is a non-profit personal fan project. The site will be kept updated.',
    copyright: lang === 'zh'
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const res = await fetch(`${BASE_PATH}/data/images.json`);
  ...
  function handleDownload(filename) {
    ...
    link.href = `${BASE_PATH}/images/full/${filename}`;
            ...
            return (
              ...
              >
                <div
                  ...
                  tabIndex={0}
                >
                  <img src={item.thumbnail ? `${BASE_PATH}/images/${item.thumbnail}` : `${BASE_PATH}/images/full/${item.full}`} alt={item.artistId || 'Denia'} loading="lazy" onError={(e) => { if (!e.currentTarget.dataset.fallbackAttempted) { e.currentTarget.dataset.fallbackAttempted = 'true'; e.currentTarget.src = `${BASE_PATH}/images/full/${item.full}`; } else { e.currentTarget.src = fallbackSvg('Denia'); } }} />
      ...
      <div className={`lightbox ${lightboxIndex >= 0 ? 'open' : ''}`} onClick={() => setLightboxIndex(-1)}>
        {currentLightboxImage ? (
          <div className="lightbox-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <ZoomableImage src={`${BASE_PATH}/images/full/${currentLightboxImage.full}`} alt={currentLightboxImage.artistId || 'Denia'} onError={(e) => { e.currentTarget.src = fallbackSvg(currentLightboxImage.artistId || 'Denia'); }} />
